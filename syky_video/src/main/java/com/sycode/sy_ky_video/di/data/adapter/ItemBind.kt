...
import com.sycode.sy_ky_video.app.utils.*
...
fun bindViewHolder(
    mContext: Context,
    datas: ArrayList<ResultEntity.ItemList>,
    ...
    when (viewHolder) {
        ...
        is ItemDynamicInfoCardItemHolder -> onItemDynamicInfoCardBind(
            mContext,
            datas,
            viewHolder,
            position
        )
        is ItemHorizontalScrollCardHolder -> onItemHorizontalScrollCardBind(
            mContext,
            datas,
            viewHolder,
            position
        ...
        is ItemVideoSmallCardHolder -> onItemVideoSmallCardBinder(
            mContext,
            datas,
            viewHolder,
            position
        )
        is ItemSquareCardCollectionHolder -> onItemSquareCardCollectionBinder(
            mContext,
            datas,
            viewHolder,
            position
        )
        is ItemVideoCollectionWithBriefHolder -> onItemVideoCollectionWithBriefBinder(
            mContext,
            datas,
            viewHolder,
            position
        ...
        is ItemVideoCollectionOfHolder -> onItemVideoCollectionOfBind(
            mContext,
            datas,
            viewHolder,
            position
...
fun onItemTextCardBind(
    mContext: Context,
    datas: ArrayList<ResultEntity.ItemList>,
    viewHolder: RecyclerView.ViewHolder,
    position: Int
) {
    val data: ResultEntity.ItemList = datas[position]
...
fun onItemDynamicInfoCardBind(
    ...
    holder.tv_dynamic_time_video!!.text =
        TimeUtils.secToTime(simpleVideo["duration"].toString().toFloat().toInt())
...
fun onItemHorizontalScrollCardBind(
    mContext: Context,
    datas: ArrayList<ResultEntity.ItemList>,
    viewHolder: RecyclerView.ViewHolder,
    position: Int
) {
    val data: ResultEntity.ItemList = datas.get(position)
...
fun onItemVideoSmallCardBinder(
    mContext: Context,
    datas: ArrayList<ResultEntity.ItemList>,
    viewHolder: RecyclerView.ViewHolder,
    position: Int
    ...
    holder.tv_time!!.text =
        TimeUtils.secToTime(videoSmallCard["duration"].toString().toFloat().toInt())
    ...
    if (videoSmallCard["author"] != null) {
        ...
        holder.tv_content!!.text =
            "#" + videoSmallCard["category"] + " / " + author["name"].toString()
                .replace(videoSmallCard["category"].toString(), "")
    }
}

fun onItemSquareCardCollectionBinder(
    mContext: Context,
    datas: ArrayList<ResultEntity.ItemList>,
    viewHolder: RecyclerView.ViewHolder,
    position: Int
    ...
    if (header["subTitle"] != null) {
        ...
        if (header["subTitleFont"] != null && "lobster" == header["subTitleFont"].toString()) {
            holder.tv_squarecard_collection_sub_title!!.typeface =
                Typeface.createFromAsset(mContext.assets, "fonts/Lobster-1.4.otf")
        }
    ...
    for (map in itemList) {
        val type = map["type"]
        val data = map["data"] as Map<*, *>
        if (MyMultiTypeAdapter.ITEM_TYPE.ITEM_FOLLOWCARD.type.hashCode() == type.toString()
                .hashCode()
        ) {
...
fun onItemVideoCollectionWithBriefBinder(
    mContext: Context,
    datas: ArrayList<ResultEntity.ItemList>,
    viewHolder: RecyclerView.ViewHolder,
    position: Int
    ...
    val data: ResultEntity.ItemList = datas[position]
    val holder: ItemVideoCollectionWithBriefHolder =
        viewHolder as ItemVideoCollectionWithBriefHolder
...
fun onItemVideoCollectionOfBind(
    mContext: Context,
    datas: ArrayList<ResultEntity.ItemList>,
    viewHolder: RecyclerView.ViewHolder,
    position: Int
    ...
    ItemClickSupport.addTo(holder.recyclerview_videocollection_of)
