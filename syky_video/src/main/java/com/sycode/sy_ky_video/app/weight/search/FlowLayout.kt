...
class FlowLayout(context: Context, attrs: AttributeSet) : ViewGroup(context, attrs) {
    ...
    ): ViewGroup.LayoutParams = ViewGroup.MarginLayoutParams(p)
        ...
        for (i in stringList.indices) {
            if (!TextUtils.isEmpty(stringList[i].trim { it <= ' ' })) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                    textView.background =
                        ContextCompat.getDrawable(context, R.drawable.btn_shap_txt)
                } else {
                    textView.setBackgroundDrawable(
                        ContextCompat.getDrawable(
                            context,
                            R.drawable.btn_shap_txt
                        )
                    )
                }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(
            if (modeWidth == View.MeasureSpec.EXACTLY)
                sizeWidth
            else
