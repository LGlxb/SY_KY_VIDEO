package com.sycode.sy_ky_video.app.weight.search
...
open class TabLayout(context: Context, attrs: AttributeSet) : HorizontalScrollView(context, attrs),
    ViewPager.OnPageChangeListener, Parcelable {
    ...
    constructor(parcel: Parcel) : this(
        ...
        textNormalSize = parcel.readFloat()
        textSelectSize = parcel.readFloat()
        textNormalColor = parcel.readInt()
        textSelectColor = parcel.readInt()
        underlineHeight = parcel.readFloat()
        underlineColor = parcel.readInt()
        indicatorHeight = parcel.readFloat()
        indicatorWidth = parcel.readFloat()
        indicatorSpacing = parcel.readFloat()
        isTextBold = parcel.readByte() != 0.toByte()
        tabPadding = parcel.readFloat()
        indicatorColor = parcel.readInt()
        tabSpaceEqual = parcel.readByte() != 0.toByte()
        mCurrentTab = parcel.readInt()
        mCurrentPositionOffset = parcel.readFloat()
        mLastScrollX = parcel.readInt()
    }
    ...
    private fun initResource(context: Context, attrs: AttributeSet) {
        ...
        textNormalSize = typedArray.getDimension(
            R.styleable.MyTabLayout_textNormalSize,
            sp2px(context, 14f).toFloat()
        )
        textSelectSize =
            typedArray.getDimension(R.styleable.MyTabLayout_textSelectSize, textNormalSize)
        ...
        underlineHeight = typedArray.getDimension(
            R.styleable.MyTabLayout_underLineHeight,
        ...
        indicatorHeight = typedArray.getDimension(
            ...
            dp2px(context, 3f).toFloat()
        )
        indicatorSpacing = typedArray.getDimension(
            R.styleable.MyTabLayout_indicatorSpacing,
            dp2px(context, 6f).toFloat()
        )
        isTextBold = typedArray.getBoolean(R.styleable.MyTabLayout_isTextBold, true)
        underlineColor =
            typedArray.getColor(R.styleable.MyTabLayout_underlineColor, Color.parseColor("#EEEEEE"))
        indicatorColor = typedArray.getColor(R.styleable.MyTabLayout_indicatorColor2, Color.BLACK)
        tabSpaceEqual = typedArray.getBoolean(R.styleable.MyTabLayout_tabSpaceEqual, false)
        tabPadding = typedArray.getDimension(
            R.styleable.MyTabLayout_tabPadding,
            (if (tabSpaceEqual) dp2px(context, 5f) else dp2px(context, 20f)).toFloat()
        )
        typedArray.recycle()
                    ...
                    if (position != -1) {
                        if (mViewPager!!.currentItem != position) {
                            ...
                            if (selectListener != null) {
                                selectListener!!.onTagSelected(
                                    position,
                                    if (mTitles != null) mTitles!![position] else mViewPager!!.adapter?.getPageTitle(
                                        position
                                    ).toString()
                                )
                            }
        ...
        for (i in 0 until tabCount) {
            ...
            if (tv_tab_title != null) {
                ...
                tv_tab_title.setTextSize(
                    ...
                    if (i == mCurrentTab) textSelectSize else textNormalSize
                )
        ...
        for (i in 0 until tabCount) {
            ...
            if (tab_title != null) {
                ...
                tab_title.setTextSize(
                    TypedValue.COMPLEX_UNIT_PX,
                    if (isSelect) textSelectSize else textNormalSize
                )
                if (isTextBold) {
    ...
    override fun writeToParcel(parcel: Parcel, flags: Int) {
        parcel.writeFloat(textNormalSize)
        parcel.writeFloat(textSelectSize)
        parcel.writeInt(textNormalColor)
        parcel.writeInt(textSelectColor)
        parcel.writeFloat(underlineHeight)
        parcel.writeInt(underlineColor)
        parcel.writeFloat(indicatorHeight)
        parcel.writeFloat(indicatorWidth)
        parcel.writeFloat(indicatorSpacing)
        parcel.writeByte(if (isTextBold) 1 else 0)
        parcel.writeFloat(tabPadding)
        parcel.writeInt(indicatorColor)
        parcel.writeByte(if (tabSpaceEqual) 1 else 0)
        parcel.writeInt(mCurrentTab)
        parcel.writeFloat(mCurrentPositionOffset)
        parcel.writeInt(mLastScrollX)
    }
    ...
    companion object CREATOR : Parcelable.Creator<TabLayout> {
        override fun createFromParcel(parcel: Parcel): TabLayout {
            return TabLayout(parcel)
        }

        override fun newArray(size: Int): Array<TabLayout?> {
