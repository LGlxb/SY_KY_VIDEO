...
class SearchView(context: Context, attrs: AttributeSet) :
    ...
    fun init(context: Context) {
        ...
        mClearDrawable!!.setBounds(
            0,
            0,
            mClearDrawable!!.intrinsicWidth * 2 / 5,
            mClearDrawable!!.intrinsicHeight * 2 / 5
        )
        //默认隐藏删除按钮
        setClearIcon(false)
     ...
     */
    fun setClearIcon(isShow: Boolean) {
    ...
    fun initResource(context: Context, attrs: AttributeSet) {
        val mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.SearchEdit)
        val density = context.resources.displayMetrics.density
        searchSize =
            mTypedArray.getDimension(R.styleable.SearchEdit_imageWidth, 18 * density + 0.5f)
    ...
    fun drawSearchIcon(canvas: Canvas) {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (compoundDrawables[2] != null) {
            if (event.action == MotionEvent.ACTION_UP) {
                val xTouchable =
                    event.x > width - paddingRight - mClearDrawable!!.intrinsicWidth && event.x < width - paddingRight

                val yTouchable =
                    event.y > (height - mClearDrawable!!.intrinsicHeight) / 2 && event.y < (height + mClearDrawable!!.intrinsicHeight) / 2
